 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *                                                                             *
 * Author: Andreas Classen <www.classen.be> - Version 20111211                 *
 *                                                                             *
	...
	if(trim($ret)==""){
		...
		switch($type) {
			...
			case "conference":
				...
				// Build output
				if(trim($booktitle)			!= "") $ret = $ret.' '.$booktitle;
				if(trim($publisher)		!= "") $ret = $ret.' <span class="publisher">'.$publisher.'</span>';
				if(trim($pubaddress)	!= "") $ret = $ret.', '.$pubaddress.'';
				if(trim($series)		!= "") $ret = $ret.', '.$series.' '.$volume.'';
				if(trim($year)			!= "") $ret = $ret.', '.$year;
			...
			case "phdthesis":
				...
				$mtype = extractBib("type", $entry, $accents);
				// Build output
			...
			case "misc":
				$note = extractBib("note", $entry,$accents);
				$howpublished = extractBib("howpublished", $entry,$accents);
				if(trim($note) != "") $ret .= ', '.$note;
				if(trim($howpublished) != "") $ret .= ', '.$howpublished;
				break;
			case "unpublished":
			case "manual":
				$note = extractBib("note", $entry,$accents);
